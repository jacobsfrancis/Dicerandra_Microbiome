...
## 10d) Select one representative within-site pair ####
#
...
## 10e) Prepare the highlighted pair in each ordination ####
...
## 10f) Draw the ordination panels ####
...
make_ordination_panel <- function(
  ...
  ggplot(
    ...
    annotate(
      ...
      hjust = -0.08,
      vjust = 1.15,
      label = example_object$line$Distance_Label,
      size = 3.5,
      fontface = "bold",
...
rhizosphere_ordination_plot <- make_ordination_panel(
  ...
  panel_title = "Rhizosphere",
  compartment_color = compartment_colors[["Soil_p"]],
  panel_letter = "B"
)

## 10g) Prepare the all-pair coordination scatterplot ####
...
## 10h) Draw the coordination scatterplot ####
#
...
## 10i) Assemble and save the conceptual figure ####
