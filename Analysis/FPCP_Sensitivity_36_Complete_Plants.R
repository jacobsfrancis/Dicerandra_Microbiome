...
## COMPLETE-PLANT SENSITIVITY ANALYSIS
## ============================================================
##
## Biological question:
...
output_directory <- file.path(
  "../Output/",
  "Sensitivity_36_Complete_Plants"
)
...
## ------------------------------------------------------------
## 9A. Restrict all analyses to plants with all seven compartments
## ------------------------------------------------------------
##
## This sensitivity analysis retains only plant individuals represented
## in every microbiome compartment. Therefore, each compartment-specific
## distance matrix contains the same host individuals.
##
## This is particularly useful while fruit sampling is incomplete because
## it prevents comparisons involving fruit from using a different subset
## of plants than comparisons involving roots, leaves, or soils.
##
## When the remaining fruit samples are complete, rerun both:
##   1. this complete-plant analysis, and
##   2. the available-sample analysis using all valid samples.

plant_compartment_coverage <- metadata_matched %>%
  distinct(
    Site,
    Plant,
    Plant_ID,
...
complete_plant_ids <- plant_compartment_coverage %>%
  filter(
    N_Compartments == length(
      compartment_order
    )
  ) %>%
  pull(
...
if (length(complete_plant_ids) < 2) {
  stop(
    paste(
      "Fewer than two plants contain all seven compartments.",
      "Inspect plant_compartment_coverage."
    )
  )
}

message(
  "Number of complete plants retained: ",
  length(
    complete_plant_ids
  )
)

complete_plants_by_site <- plant_compartment_coverage %>%
  filter(
    Plant_ID %in% complete_plant_ids
  ) %>%
  distinct(
    Site,
    Plant,
...
retained_sample_wide <- retained_sample_summary %>%
  pivot_wider(
...
print(
  retained_sample_wide,
  width = Inf
)

# Confirm that every compartment contains the same plant IDs.
complete_plant_check <- metadata_matched %>%
  distinct(
    Plant_ID,
    Compartment
  ) %>%
...
if (
  any(
    complete_plant_check$N_Compartments !=
      length(
        compartment_order
      )
  )
  ...
  stop(
    paste(
      "Complete-plant filtering failed:",
      "at least one retained plant does not contain all compartments."
    )
  )
}

compartment_sample_check <- metadata_matched %>%
  count(
    Compartment,
    name = "N_Samples"
  )
...
if (
  n_distinct(
    compartment_sample_check$N_Samples
  ) != 1
) {
  stop(
    paste(
      "Compartment sample sizes differ after complete-plant filtering.",
      "Inspect compartment_sample_check."
    )
  )
}
...
write_csv(
  plant_compartment_coverage,
  file.path(
    output_directory,
    "plant_compartment_coverage_before_restriction.csv"
  )
)
...
write_csv(
  ...
  file.path(
    output_directory,
    "complete_plants_retained_by_site.csv"
  )
)
...
write_csv(
  ...
  file.path(
    output_directory,
    "retained_samples_complete_plants.csv"
  )
)
...
run_site_permanova <- function(
  ...
  tibble(
    ...
    Dispersion_P = as.numeric(
      dispersion_table[[dispersion_p_column]][1]),
...
site_r2_plot <- ggplot(
  ...
  labs(
    ...
    caption = paste(
      "All compartments use the same complete plant subset;",
      "site sample sizes are reported in the exported results table."
    )
  ) +
  theme_minimal(
    base_size = 12
...
within_among_plot <- ggplot(
  ...
  labs(
    ...
    y = NULL,
    caption = paste(
      "All compartments use the same",
      length(complete_plant_ids),
      "complete plant individuals."
    )
  ) +
  guides(
    fill = guide_legend(
